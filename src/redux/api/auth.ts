...
import { authenticated_ok, fail_clear, fail_user, loaded_user, login_ok, off_loading, on_loading, refresh_ok } from "../slice/authSlice";
...
export const load_user = () => async (dispatch: AppDispatch) => {
    if (getStoreLocal('access')) {
        await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/auth/users/me/`,
            {
                headers: {
...
export const signup = (first_name: string, last_name: string, email: string, password: string, re_password: string) => async (dispatch: AppDispatch) => {
    ...
    await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/auth/users/`, JSON.stringify({
        first_name,
        last_name,
        email,
        password,
...
export const activate = (uid: (string | string[] | undefined), token: (string | string[] | undefined)) => async (dispatch: AppDispatch) => {
    dispatch(on_loading());
    await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/auth/users/activation/`, JSON.stringify({
        uid,
        token
    }), {
        headers: {
